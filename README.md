...
날씨 관측데이터의 전처리와, model 설계를 중점적으로 담당하였다. 

-관측 데이터에서 ‘세종’ 지역은 2020년부터 수집되었기 때문에, 학습에 필요한모든 기간의 날씨 관측 데이터가 결측이었다. 따라서 세종지역의 데이터는 세종 근방의 관측 지점들의 평균값으로 대체하였다.

-지역별로 나누어진 데이터로 행의 개수가 충분하지 않아, 부스팅이나 딥러닝 보다 단순한 회귀 모형이 더 적절할 것이라 가설을 세웠고, 그 결과도 일치하였다. 최종 모형은 Bayesian regression, lasso 위주의 앙상블 모형이 채택되었다.


# 어려웠던점, 해결방법

“기상”과 심뇌혈관질환자의 “입원 건수” 사이의 상관관계가 낮아서 모델 성능 향상에 있어서 어려움을 느꼈다.

EDA 결과 하루 입원건수는 지역별, 성별, 일자별에 따라 서로 다른분포를 보임을 확인할 수 있었고, 이 차이를 모델에 반영하기 위해 회귀 모델에 지역별, 성별, 일자별 평균값을 가중치로 부여한 최종 모델을
생성하였다. 그 결과 예측 성능이 크게 향상됨을 확인할 수 있었다. 

# 배운점/ 성과

- 2022 기상청 빅데이터 콘테스트 장려상(예측 성능 2위) 수상

- 단순 성능 향상을 위해 다양한 파생변수를 추가하는 것 보다 프로젝트 목적에 맞게 주제와 관련된 변수들을 이용해 분석을 진행하는 것이 중요함을 경험할 수 있었다. 
